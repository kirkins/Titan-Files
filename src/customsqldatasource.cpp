/* Modified by Taylor Byrnes to provide paramteter binding support
 * Original copyright (c) 2012 Research In Motion Limited.
 *
...
QVariant CustomSqlDataSource::executeAndWait(const QString &query, const QVariantList &valuesByPosition, int id)
{
    DataAccessReply reply;

    if (checkConnection()) {
        reply = mSqlConnector->executeAndWait(query, valuesByPosition, id);
...
void CustomSqlDataSource::execute (const QString &query, const QVariantList &valuesByPosition, int id)
{
    if (checkConnection()) {
        mSqlConnector->execute(query, valuesByPosition, id);
    }
}
...
void CustomSqlDataSource::load()
{
    ...
    if (mQuery.isEmpty() == false) {
    	execute(mQuery, list, LOAD_EXECUTION);
    }
}

void CustomSqlDataSource::load(const QString &query, const QVariantList &valuesByPosition) {
	execute(query, valuesByPosition, LOAD_EXECUTION);
}
